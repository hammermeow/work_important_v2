DROP VIEW nvrpt.cost_lever_hours_cp;

CREATE OR REPLACE VIEW nvrpt.cost_lever_hours_cp
(
employee_id,
login_name,
supervisor_id,
...
duration_hours,
role,
weekly_staff_hrs_raw_data,
weekly_staff_hrs_cal_by_buz_day,
manager_flag,
hc_role,
job_level,
is_active,
...
FROM (
---------------------------------------aai Start -------------------------------------------------------------------------------------
SELECT CAST(user_id AS TEXT) "employee_id",
      ...
      SELECT user_id,
                   ...
                   CASE
                     ...
                     ELSE INITCAP(REPLACE(
                       ...
                       END ,'_',' '))
                   END AS current_status, --Current Status
                   ...
                   END AS status_l3,--status_l3
                   map.*
            FROM nvrpt.fact_da_availability_daily ava
              LEFT JOIN nvads.da_mapping_table MAP ON ava.status_i18n_id = map.level3 --from level3 get level 2 and level 1
            where TRUNC(start_datetime) > sysdate-750
            ) fa,
            -----------------------------------fa end---------------------------------------------
  ...
  FULL OUTER JOIN (SELECT *
                   FROM (SELECT user_id,
                                ...
                                wud.weekly_staff_hrs as weekly_staff_hrs_raw_data,
                                wud.weekly_staff_hrs * (business_days / 5::FLOAT) AS weekly_staff_hrs_cal_by_buz_day, --staff hour calculation
                                CASE
                         ...
                         FROM nvdev.wbr_user_details_hist wud
                           JOIN nvdev.wbr_week_cal wwc
                            ...
                            AND wwc.end_date
                         WHERE wud.file_date >= sysdate-750
                         )
                   WHERE RANK_no = 1) a
...
GRANT SELECT ON nvrpt.cost_lever_hours_cp TO nvort;
